// V2.1 expanded findings postprocessing — canonical implementation.
//
// Lives under `_shared/` so both `analyze-voice-patterns` (App) and
// `analyze-voice-patterns-shared` (Doctor-Share) can import it via a
// path that the Supabase Edge Function bundler can resolve (sibling
// function folders are NOT bundled together, so cross-function imports
// like `../analyze-voice-patterns/postprocess.ts` fail at deploy time).
//
// The legacy file `analyze-voice-patterns/postprocess.ts` is now a thin
// re-export of this module so the existing Deno tests keep working.
