...
/**
 * Compute pressure_change_24h directly from Open-Meteo Archive.
 * Strategy: fetch hourly surface_pressure for [date-1, date], pick the index
 * closest to `at` and the index 24h earlier, return rounded Δ.
 * Returns null if archive data is missing/invalid.
 */
async function fetchPressureDelta24hFromArchive(
  ...
  try {
    ...
    const pickClosest = (targetMs: number): number | null => {
      ...
      // Reject matches > 3h away — protects against sparse archive
    ...
    console.log('⚠️ Δ24h archive call failed:', err);
    return null;
  }
}
