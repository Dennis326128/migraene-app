-- Performance indices
CREATE INDEX IF NOT EXISTS pain_entries_user_time_idx 
  ON pain_entries (user_id, timestamp_created);
CREATE INDEX IF NOT EXISTS pain_entries_pain_level_idx 
  ON pain_entries (pain_level);

-- Aggregations view (daily stats in Europe/Berlin timezone)
CREATE OR REPLACE VIEW v_migraine_daily AS
SELECT
  user_id,
  (timestamp_created AT TIME ZONE 'Europe/Berlin')::date as day_berlin,
  count(*) as cnt,
  avg(case pain_level
        when 'leicht' then 2 when 'mittel' then 5
        when 'stark' then 7 when 'sehr_stark' then 9
        else 0
      end)::numeric(4,2) as avg_score,
  sum(case when medications is not null and array_length(medications, 1) > 0 then 1 else 0 end)::int as with_meds
FROM pain_entries
WHERE pain_level != '-'
GROUP BY 1, 2;

-- RPC: Filtered entries
...
-- RPC: Time distribution (hours of day)
