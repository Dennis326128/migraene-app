-- SECURITY FIX: Create only missing lifestyle_logs and hormonal_logs tables

-- 1. Create lifestyle_logs table with full RLS
...
-- Create RLS policies for lifestyle_logs (with IF NOT EXISTS check)
DO $$
BEGIN
...
-- 2. Create hormonal_logs table with full RLS
...
-- Create RLS policies for hormonal_logs (with IF NOT EXISTS check)
DO $$
BEGIN
...
-- 4. Add update triggers for updated_at columns
DO $$
BEGIN
