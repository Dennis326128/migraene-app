...
import { Activity, MapPin, Pill, Info } from "lucide-react";
  ...
  return (
    <div className="mt-2 pt-2 border-t border-border/50">
      ...
      <div className="flex items-center justify-between text-sm mb-1">
        ...
        {isOver ? (
          <span className="text-xs font-semibold text-destructive">+{limitInfo.overBy} über</span>
        ) : (
          <span className="text-xs text-muted-foreground">{limitInfo.remaining} übrig</span>
        )}
      </div>
      <Progress 
        value={percentage} 
      ...
      <div className="mt-1 flex items-center gap-2 text-sm">
        {med.avgEffect !== null ? (
          <>
            ...
            <span className="text-xs text-muted-foreground">({getEffectLabel(Math.round(med.avgEffect))})</span>
          </>
        ) : (
      ...
      {showLimit && med.limitInfo && (
        <>
          ...
          {med.limitInfo.isOverLimit && onNavigateToLimits && (
            <Button variant="outline" size="sm" className="h-7 text-xs mt-2" onClick={onNavigateToLimits}>
              Limits-Übersicht
            </Button>
          )}
        </>
...
export function PatternCards({ statistics, isLoading = false, overuseInfo, daysInRange, reportKpis }: PatternCardsProps) {
  ...
  return (
    <div className="space-y-4">
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="text-base">Schmerzintensität</CardTitle>
            </div>
      ...
      {/* 2. Medikamente & Wirkung – volle Breite */}
      <Card>
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {painLocation.mostCommon ? (
              ...
              <div className="space-y-1.5 text-sm">
                {painLocation.distribution.slice(0, 4).map((item, idx) => (
                  <div key={idx} className="flex justify-between">
                    <span className="text-muted-foreground">{formatPainLocation(item.location)}:</span>
            ...
            <p className="text-sm text-muted-foreground">Keine Lokalisationsdaten im Zeitraum</p>
          )}
        </CardContent>
      </Card>
    </div>
