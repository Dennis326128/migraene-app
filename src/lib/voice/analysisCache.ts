...
export type StaleReason = 'data_changed' | 'version_mismatch' | 'range_mismatch' | null;

export interface AnalysisSelection {
  /** The analysis result, or null if none available */
  result: VoiceAnalysisResult | null;
  /** Whether the analysis matches the current data state AND analysis version */
  isFresh: boolean;
  ...
  /**
   * If status is 'stale_accepted', why is it stale?
   *  - 'version_mismatch': stored signature has no `v:` prefix or different ANALYSIS_VERSION
   *  - 'data_changed': same version, but pain/voice/medication data changed since.
   *  - 'range_mismatch': no analysis exists for the exact selected range; the most
   *    recent analysis from another range is shown as a fallback.
   * null when status !== 'stale_accepted'.
   */
  staleReason: StaleReason;
  /** True when the returned result is from a different date range than requested. */
  isRangeFallback?: boolean;
  /** Date range of the returned analysis (may differ from requested when isRangeFallback). */
  resultFromDate?: string | null;
  resultToDate?: string | null;
}

/**
 * Extract the analysis version embedded in a state signature.
 * Format: "v:<version>|...". Returns null for legacy signatures without prefix.
 */
export function extractSignatureVersion(sig: string | null): string | null {
  if (!sig) return null;
  const m = /^v:([^|]+)\|/.exec(sig);
  return m ? m[1] : null;
}
...
export async function selectAnalysisForChannel(
  ...
  if (!fingerprint) {
    return { result: null, isFresh: false, status: 'not_authenticated', storedSignature: null, currentSignature: null, staleReason: null };
  }
  ...
  // Distinguish version mismatch from data change.
  const storedVersion = extractSignatureVersion(cached.dataStateSignature);
  const currentVersion = extractSignatureVersion(fingerprint.stateSignature);
  const staleReason: StaleReason =
    (storedVersion === null || storedVersion !== currentVersion)
      ? 'version_mismatch'
      : 'data_changed';

  // Stale: all channels accept (stale > missing), UI decides presentation.
  ...
  return {
    ...
    storedSignature: cached.dataStateSignature,
    currentSignature: fingerprint.stateSignature,
    staleReason: staleAccepted ? staleReason : null,
  };
}
